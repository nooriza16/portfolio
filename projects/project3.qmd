---
title: "London's Tube Resilience"
date: 2025-02-10
description: "Scenario investigating the resilience of the London Underground during morning peak hours (07:00–10:00 AM)"
...
Have you ever experienced using public transport but it does not operate the way you thought it would? You check upon your maps and......voila diversion. Either you have to wait, which is unlikely on some cases, or change into other transports choice. What if those scenario happened in the most important node of transport, or in a peak hour? In this post, I build a mitigation scenario when disruption hits the Tube network. The project build on by first identifying which stations are most important in the network. Secondly, we model the network cohesion by removing the 10 most important stations sequentially. Lastly, when the flow considered which station is being impacted the most by the closure scenario and what is the alternative without overwhelming the rest of the network.

## **Dataset**

1.  London Tube Netwowk (Simplified into a single line)
2.  Londons Passenger's flow

## Analytics

Before we jump into the scenario, we have first propose a question: what is the definition of importance in this regard? is it the most connected one? the longest line? or the most used for transit? In NetworkX library we can answer all of those 'importance' questions. This analysis is a simplified model of TFL's network where there will be no distinction between lines and there will be no consideration of changes in the station.
...
**Remark :** The analysis is carried out using NetworkX library in python environment, learned during urban simulation course in CASA UCL. The GitHub repository is available upon requested (or I will update the script here later), feel free to email me !
...
Göransson, J. and Andersson, H. (2023) ‘Factors that make public transport systems attractive: a review of travel preferences and travel mode choices’, *European Transport Research Review*, 15(1), p. 32. doi: 10.1186/s12544-023-00609-x.

Newman, M. (2010) *Networks*. Oxford University Press. doi: 10.1093/acprof:oso/9780199206650.001.0001.
