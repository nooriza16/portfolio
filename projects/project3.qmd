...
Have you ever experienced using public transport, it does not operate the way you thought it would? You check upon your maps and voila diversion. Either you have to wait, which is unlikely on some cases, or change into other transports choice. What if those scenario happened in the most important node of transport, or so in a peak hour?

To make the impacts less disruptive, I build a mitigation scenario to first identify which stations are most important in the network, therefeore when there is a disruption them in the most critical hours we could provide alternatives for passengers without overwhelming the rest of the network. So how do we do this?
...
Before we jump into the scenario, we have first to ask ourselves a question what does the most important station mean? is it the most connected one? the longest line? or the most used for transit? In using NetworkX in python environment we can answer all of those 'importance' questions.
