...
Have you ever used public transport, only to find it doesn't operate the way you expected? You check your maps app, and... voilà...a diversion. Either you have to wait, which isn't always guaranteed to help, or switch to another mode of transport. But what if that scenario happened at the most important node in the network, or during peak hour? In this post, I build a mitigation scenario for disruptions hitting the Tube network. The project begins by identifying which stations are most important in the network. Second, I model network cohesion by sequentially removing the 10 most important stations. Finally, I examine which stations are impacted the most by each closure scenario, and identify alternative routes that avoid overwhelming the rest of the network
...
Before we dive into the scenario, we must first pose a question: what does "importance" mean in this context? Is it the most connected station? The one on the longest line? Or the one most used for transit? Using the NetworkX library, we can answer all of these "importance" questions. This analysis is a simplified model of TfL's network, in which there is no distinction between lines and no consideration of interchanges at stations.
